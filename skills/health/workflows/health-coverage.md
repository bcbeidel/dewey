...
## Step 2: Scan knowledge/ contents

Walk the `knowledge/` directory structure. Build a map of:

- **Domain areas** -- Directories under `knowledge/` (excluding `_proposals/` and other `_` prefixed dirs)
...
Compare AGENTS.md responsibilities against knowledge/ contents:
...
For each domain area and topic in knowledge/, check whether it connects to a responsibility in AGENTS.md. Flag content with no matching responsibility:
