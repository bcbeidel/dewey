...
Verify that `knowledge/` exists under the KB root. If not, report: "No knowledge/ directory found at `<path>`. Is this a knowledge base? Use `/dewey:init` to create one."

## Step 2: Run Tier 1 validators

```bash
python ${CLAUDE_PLUGIN_ROOT}/skills/health/scripts/check_kb.py --kb-root <kb_root>
```
